...
### 4. 公式お知らせ (cloud.sakura.ad.jp/news) を確認

GitHub のリリースノートだけでは拾えない、**OSS 利用者に影響の大きい告知**が公式のお知らせとして出ていることがある。対象月のお知らせを確認し、該当するものは記事に取り込む。

- 確認先: `https://cloud.sakura.ad.jp/news/<YYYY>/<MM>/` (例: `2026/06/`)。WebFetch で対象月の見出しを一覧し、sacloud OSS 関連のものを開く
- 特に拾うべき告知の例:
  - **Go SDK や CLI の統合・廃止 (EOL)・リポジトリのアーカイブ化** (例: 2026 年 6 月の「Go SDK 群を `sacloud-sdk-go` に統合」)。直接 import している利用者に移行を促す必要があるので重要トピックとして扱う
  - プロバイダー/ツールのメジャーバージョン方針、メンテナンス終了の告知
  - 新プロダクトの提供開始 (OSS 対応の前提になる)
- 取り込むときは、**いつ何が起きるか (アーカイブ予定日など)・対象リポジトリ・利用者がすべき対応**を具体的に書き、お知らせ URL をインライン + bare URL (カード化) で 1 本貼る
- このクラスの告知は GitHub リリースより影響範囲が広いことが多いので、リード文・タイトル・専用セクションでの扱いを検討する
...
4. **リンク切れ・内容の裏付けを確認する** (必須)。記事内の全 URL を抽出し、すべて疎通すること・リンク先に記事の記述どおりの情報が載っていることを照合する。捏造リンクや存在しないバージョンへのリンクを残さない。
   ```bash
   # 記事から URL を抽出
   grep -oE 'https://[^ )]+' articles/<slug>.md | sort -u

   # リリースタグ URL は gh api でタグの存在を確認 (200 なら .tag_name が返る)
   gh api "repos/sacloud/<repo>/releases/tags/<tag>" --jq '.tag_name'

   # blob / contents へのリンクはファイルの存在を確認
   gh api "repos/sacloud/<repo>/contents/<path>?ref=<branch>" --jq '.name'
   ```
   - **疎通確認** — すべての URL が 200 で解決すること (404 のリンクは記事から外すか正しい URL に直す)
   - **内容の照合** — 各リリースタグについて `gh release view <tag> -R sacloud/<repo> --json body` の本文に、記事で書いた機能追加・バグ修正の記述が実際に含まれているかを確認する。リリースノートに無いことを書かない
   - bash の `for` ループは cwd リセットで壊れることがあるので、確認用コマンドは関数化するか 1 URL ずつ実行すると確実
5. 完了後、生成パスをユーザに報告し「frontmatter の `published: false` のまま下書きとして残しています。内容を確認のうえ `true` に変えてください」と添える
